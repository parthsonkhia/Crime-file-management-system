...
<body>
	...
	<div class="wrapper">
		...
		<div class="content">
            ...
            <div class="area1">
            <center><u>Delete Judge</u></center>
                  
                 <form action="deletej.php" method="POST" class="form1">
                  <center>
                       <table class="table2">
                      
                        <tr>
                              <td>Select Judge Name</td>
                              <td>
                                   <select placeholder="enter user name">
                                    <option></option>
                                    <?php
                                    require '../connect.php';
                                    $query=mysqli_query($conn,"SELECT * FROM `judge`");
                                    while ($row=mysqli_fetch_array($query)) {
                                     $j_name=$row['j_name'];
                                     echo "<option>$j_name</option>";
                                    }
                                    ?>
            	...
            	extract($_POST);
            	if (isset($btn)) {
            		$query=mysqli_query($conn,"DELETE FROM `judge` WHERE `j_name`='$j_name'");
            		if (!empty($query)) {
            			echo "Judge Deleted...<br><br>

                  <a href='deletej.php'>refresh page</a>";
            		}
            		else	
            		{
